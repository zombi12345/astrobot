...
async def show_admin_panel(message_or_callback):
    ...
    if isinstance(message_or_callback, Message):
        await message_or_callback.answer("👑 **Админ-панель**\n\nВыберите действие:", 
                                         reply_markup=keyboard, 
                                         parse_mode="Markdown")
    else:
        await message_or_callback.message.edit_text("👑 **Админ-панель**\n\nВыберите действие:", 
                                                   reply_markup=keyboard, 
                                                   parse_mode="Markdown")

@router.callback_query(F.data == "admin_panel")
async def admin_panel_callback(callback: CallbackQuery):
    """Открывает админ-панель из главного меню"""
...
async def admin_give_sub_start(callback: CallbackQuery):
    ...
    # Сохраняем состояние в callback.data (будет обработано отдельным обработчиком)
    # Для простоты пока пропустим полноценную выдачу, можно добавить позже
    await callback.message.answer("В разработке. Для выдачи подписки используйте /admin и выберите пользователя из списка.")
