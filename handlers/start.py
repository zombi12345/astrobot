...
from utils.helpers import md2_escape  # вместо from utils import md2_escape
...
async def start_handler(message: Message):
    ...
    text = md2_escape(f'Добро пожаловать в AstroBot, {first_name}!')
...
async def check_status(callback: CallbackQuery):
    ...
    if is_paid:
        ...
        await callback.message.edit_text('Выберите тариф:', reply_markup=payment_keyboard())
...
async def payment_handler(callback: CallbackQuery):
    ...
    await callback.message.edit_text('Подписка активирована!', reply_markup=main_menu_keyboard(callback.from_user.id))
